      ...
      for (i = 0; i < room_day_patient_list[r][d].size(); i++)
      {
        ...
        if (p < in.Patients())
        {
          ...
          if (in.PatientSkillLevelRequired(p, s1) > in.NurseSkillLevel(n))
          {
            ...
            if (VERBOSE)
              cout << "Nurse " << in.NurseId(n) << " is underqualified for patient " << in.PatientId(p) << " in room " << in.RoomId(r) << " in shift " << in.ShiftDescription(s) << endl;
          }
        }
        else if (in.OccupantSkillLevelRequired(p - in.Patients(), s) > in.NurseSkillLevel(n))
        {
          ...
          if (VERBOSE)
            cout << "Nurse " << in.NurseId(n) << " is underqualified for occupant " << in.OccupantId(p - in.Patients()) << " in room " << in.RoomId(r) << " in shift " << in.ShiftDescription(s) << endl;
        }
      }
    }
  return cost;
...
int IHTP_Output::AdmissionDay() const
{
  ...
  for (int p = 0; p < in.Patients(); p++)
    if (admission_day[p] != -1 &&
        (admission_day[p] < in.PatientSurgeryReleaseDay(p) || (admission_day[p] > in.PatientLastPossibleDay(p))))
    {
      cost++;
      if (VERBOSE)
      {
  ...
  for (n = 0; n < in.Nurses(); n++)
    for (k = 0; k < in.NurseWorkingShifts(n); k++)
    {
      ...
      if (load > in.NurseMaxLoad(n, s))
      {
        ...
        if (VERBOSE)
          cout << "Excessive workload  " << load - in.NurseMaxLoad(n, s) << " for nurse " << in.NurseId(n) << " in shift " << in.ShiftDescription(s) << endl;
      }
    }
  return cost;
}
